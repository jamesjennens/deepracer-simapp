...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface, AbstractTracker):
    ...
    def __init__(self, config_dict, run_phase_sink, metrics):
        ...
        # Normalized start position offset w.r.t to the finish/start line
        self._start_ndist_offset = start_ndist
    ...
    def update_agent(self, action):
        ...
        self._reward_params_[const.RewardParam.SIM_TIME.value[0]] = rospy.get_time()
        set_reward_and_metrics(self._reward_params_, self._step_metrics_,
                               ...
                               current_car_pose, self._start_ndist_offset)
    ...
    def judge_action(self, agents_info_map):
        ...
        if self._ctrl_status[AgentCtrlStatus.AGENT_PHASE.value] == AgentPhase.RUN.value:
            self._pause_duration = 0.0
            try:
                print(self._reward_params_['timestamp'], rospy.get_time(), self._metrics._start_time_, time.time())
                LOGGER.info('timestamps:%.4f,%.4f,%.4f,%.4f' % (self._reward_params_['timestamp'], rospy.get_time(), self._metrics._start_time_, time.time()))
        ...
        self._step_metrics_[StepMetrics.TIME.value] = self._metrics._start_time_ + rospy.get_time()
