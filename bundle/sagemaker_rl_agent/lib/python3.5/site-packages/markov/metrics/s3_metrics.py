...
def write_metrics_to_s3(bucket, key, region, metrics, endpoint_url=None ):
    ...
    try:
        ...
        s3_client = session.client('s3', region_name=region, endpoint_url=endpoint_url, config=get_boto_config())
...
class TrainingMetrics(MetricsInterface, ObserverInterface):
    ...
    def __init__(self, agent_name, s3_dict_metrics, s3_dict_model, ckpnt_dir, run_phase_sink, use_model_picker=True):
        ...
        self._start_time_ = rospy.get_time()
        self._episode_ = 0
        ...
        self._best_chkpnt_stats = {'name': None, 'avg_comp_pct': -1.0, 'time_stamp': rospy.get_time()}
...
class EvalMetrics(MetricsInterface):
    ...
    def append_episode_metrics(self):
        ...
        eval_metric['metric_time'] = int(round(sim_time * 1000))
